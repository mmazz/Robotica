...
teleop::Teleop::Teleop(void) : n("~"), key_override_enabled(false), joy_override_enabled(false)
{
  ...
  else key_axes = { { "pitch+", 264 }, { "pitch-", 258 }, { "yaw+", 257 }, { "yaw-", 259 }, { "roll+", 260 }, { "roll-", 262 }, { "vertical+", 263 }, { "vertical-", 265 } };
  for (auto& k : key_axes) key_axes_map[k.second] = k.first;
  key_axes_state = { { "pitch", 0 }, { "yaw", 0 }, { "roll", 0 }, { "vertical", 0 } };
  ...
  /* subscribe to lower priority control publisher */
  vel_sub = n.subscribe("/teleop/cmd_vel", 1, &Teleop::velocity_command, this);
  ...
  /* publish events and control commands */  
	pub_vel = n.advertise<geometry_msgs::Twist>("/robot/cmd_vel", 1);
...
void teleop::Teleop::velocity_command(const geometry_msgs::Twist::ConstPtr& vel)
{
  if (joy_override_enabled or key_override_enabled)
    return;
...
void teleop::Teleop::joystick_event(const sensor_msgs::Joy::ConstPtr& joy)
{
  ...
  for (uint32_t b = 0; b < joy->buttons.size(); b++) {
    if (joy->buttons[b] != last_joy_msg.buttons[b]) {
      ...
      if (joy_button_map.find(b) == joy_button_map.end()) continue;
  ...
  for (uint32_t a = 0; a < joy->axes.size(); a++) {
    if (joy->axes[a] != last_joy_msg.axes[a]) {
      ...
      if (joy_axis_map.find(a) == joy_axis_map.end()) continue;
			else c.controls =	{ joy_axis_map[a] };

			if (std::abs(joy->axes[a]) < joy_deadzones[joy_axis_map[a]])
				c.values = { 0 };
      else {
				c.values = { joy->axes[a] };
      }
...
void teleop::Teleop::keyboard_up_event(const keyboard::Key::ConstPtr& key)
{
  ...
  if (key_axes_map.find(key->code) != key_axes_map.end()) {
    std::string& cmd = key_axes_map[key->code];
		std::string axis_name = cmd.substr(0, cmd.size() - 1);
    key_axes_state[axis_name] = 0;

		teleop::Control c;
		c.header = key->header;
		c.controls = { axis_name };
		c.values = { 0 };
		pub_control.publish(c);
  }
  else {
    ...
    if (key_map.find(key->code) == key_map.end()) return;
...
void teleop::Teleop::keyboard_down_event(const keyboard::Key::ConstPtr& key)
{
  ...
  if (key_axes_map.find(key->code) != key_axes_map.end()) {
    std::string& cmd = key_axes_map[key->code];
		std::string axis_name = cmd.substr(0, cmd.size() - 1);
    key_axes_state[axis_name] = (cmd[cmd.size() - 1] == '+' ? 1 : -1);

		teleop::Control c;
		c.header = key->header;
		c.controls = { axis_name };
		c.values = { (float)key_axes_state[axis_name] };
		pub_control.publish(c);
  }
  else {
    teleop::Event e;
...
void teleop::Teleop::control(void)
{
  if (joy_override_enabled) {
    ...
    vel.linear.y = key_axes_state["roll"] * axis_scales["roll"];
    vel.linear.z = key_axes_state["vertical"] * axis_scales["vertical"];
    //vel.angular.x = vel.angular.y = 1; // non-zero to avoid hovering when zero-ing controls
